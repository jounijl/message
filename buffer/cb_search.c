...
int  cb_put_leaf(cb_name **tree, cb_name **leaf, int openpairs); // not yet tested 7.12.2013
...
int  cb_set_to_leaf(CBFILE **cbs, cb_name **leaf, cb_name **result, unsigned char **name, int namelen, int openpairs, int matchctl){
	...
	if(cbs==NULL || *cbs==NULL || (**cbs).cb==NULL || result==NULL || name==NULL ){
	...
	if( leaf==NULL || *leaf==NULL)
	  return CBEMPTY;
	// Node
	err = cb_compare( &(*cbs), &(*name), namelen, &(**leaf).namebuf, (**leaf).namelen, matchctl);
	if(err==CBMATCH){
	  if( openpairs == 2 ){ // 2 = first leaf
	    (*result) = &(**leaf);
	    // SET_CURSOR
	    /*
	     ...
	     *
	     * (1) Saako tassa asettaa current:in ? (2) Tuleeko olla currentleaf ?
	     */
	    fprintf(stderr, "\ncb_set_to_leaf: debug: SET_CURSOR."); // debug
	    (**result).matchcount++; if( (**result).matchcount==0 ){ (**result).matchcount+=2; }
	    if( ( (**cbs).cf.searchmethod==CBSEARCHNEXTNAMES && (**result).matchcount==1 ) || (**cbs).cf.searchmethod==CBSEARCHUNIQUENAMES ){ 
	      if( (**cbs).cf.type!=CBCFGFILE ) // When used as only buffer, stream case does not apply
	        if((**result).offset>=( (*(**cbs).cb).buflen + 0 + 1 ) ){ // buflen + smallest possible name + endchar
		  /*
	           * Leafs content was out of memorybuffer, setting
		   * CBFILE:s index to the edge of buffer.
	 	   */
	          (*(**cbs).cb).index = (*(**cbs).cb).buflen; // set as a stream, 1.12.2013
	          return CBNAMEOUTOFBUF;
	        }
	      (*(**cbs).cb).index=(**result).offset; // 1.12.2013
	    }
	    // / SET_CURSOR
	...
	if( (**leaf).leaf!=NULL && openpairs>=2 ){ // Left
	  leafptr = &(* (cb_name*) (**leaf).leaf);
	  pairsleft = openpairs - 1;
	}else if( (**leaf).next!=NULL ){ // Right
	  leafptr = &(* (cb_name*) (**leaf).next);
	}else
	  return CBNOTFOUND;
	return cb_set_to_leaf( &(*cbs), &leafptr, &(*result), &(*name), namelen, pairsleft, matchctl);
}
...
int  cb_put_leaf(cb_name **tree, cb_name **leaf, int openpairs){
	 ...
	 * Find last leaf */
	leafcount = openpairs;
	err = cb_set_to_last_leaf( &(*tree), &lastleaf, &leafcount);
	...
	//fprintf(stderr,"\ncb_put_leaf: leaf put was: [");
	//cb_print_name(&newleaf);
	//fprintf(stderr,"], err=%i, ret=%i, leafcount=%i, openpairs=%i, returning.", err, ret, leafcount, openpairs);
...
int  cb_put_name(CBFILE **str, cb_name **cbn, int openpairs){ 
	  ...
	  if(openpairs>1){ // first '=' is openpairs==1 (used only in CBSTATETREE)
	    err = cb_put_leaf( &(*(**str).cb).list.current, &(*cbn), openpairs); // openpairs chooses between *next and *leaf
            //(*(**str).cb).list.currentleaf = NULL; // 9.12.2013
	    return err;
	  }
	  ...
	  while(iter != NULL){
	    ...
	    if( err == CBMATCH ){ // 9.11.2013
	      ...
	      if( ( (**str).cf.searchmethod==CBSEARCHNEXTNAMES && (*iter).matchcount==1 ) || (**str).cf.searchmethod==CBSEARCHUNIQUENAMES ){ 
	        ...
	        //(*(**str).cb).list.current=(cb_name*) iter; // 1.12.2013
	        (*(**str).cb).list.current=&(*iter); // test 1.12.2013
	        return CBSUCCESS;
	      }
	    }
...
int  cb_set_cursor_match_length(CBFILE **cbs, unsigned char **name, int *namelength, int matchctl){
	...
	err = cb_set_cursor_match_length_ucs( &(*cbs), &ucsname, &chrbufindx, matchctl);
	free(ucsname);
	return err;
}
int  cb_set_cursor(CBFILE **cbs, unsigned char **name, int *namelength){
	return cb_set_cursor_match_length( &(*cbs), &(*name), &(*namelength), 1 );
}
...
int  cb_set_cursor_ucs(CBFILE **cbs, unsigned char **ucsname, int *namelength){
	return cb_set_cursor_match_length_ucs( &(*cbs), &(*ucsname), &(*namelength), 1 );
}
...
cb_set_cursor_reset_name_index:
	index=0;
	//err = cb_allocate_name(&fname); // moved to cb_save_name_from_charbuf 7.12.2013
	...
	while( err<CBERROR && err!=CBSTREAMEND && index < (CBNAMEBUFLEN-3) && buferr <= CBSUCCESS){ 
	  ...
	  // doubledelim is not tested yet 8.12.2013
	  if( (**cbs).cf.searchstate==CBSTATETOPOLOGY || (**cbs).cf.searchstate==CBSTATETREE ){ // '=', save name
	    if( ( chr==(**cbs).cf.rstart || ( chr==(**cbs).cf.subrstart && !intiseven( openpairs ) && (**cbs).cf.doubledelim==1 ) ) && \
	      ...
	      if( (**cbs).cf.searchstate==CBSTATETREE ){ // saves leafs
	        tovalue=1;
	      }
	    }
	  ...
	  if(tovalue==1){
	    ...
	    if( buferr==CBSUCCESS ){
	      ...
	      if(openpairs>=1) // CBSTATETREE, reset charbuf index, 7.12.2013
	        goto cb_set_cursor_reset_name_index;
	      if( (**cbs).cf.searchstate==CBSTATELESS && (**cbs).cf.statelessinnervalues==1) // From '=' to '=' and from '&' to '=',
	    ...
	    if(buferr==CBNAMEOUTOFBUF || buferr>=CBNEGATION){
	      ...
	      if((**cbs).cf.searchstate==CBSTATETOPOLOGY){
	        fprintf(stderr, "(CBSTATETOPOLOGY).");
	      }else if((**cbs).cf.searchstate==CBSTATEFUL){
	        fprintf(stderr, "(CBSTATEFUL).");
	      }else if((**cbs).cf.searchstate==CBSTATETREE){
	        fprintf(stderr, "(CBSTATETREE).");
	      }else if((**cbs).cf.searchstate==CBSTATELESS){
	        fprintf(stderr, "(CBSTATELESS).");
	      }else
	        fprintf(stderr, ".");
	    }
	    ...
	    if(openpairs<=0){ // 2.12.2013, leaves are compared elsewhere (name1.name2.name3: only name1 matches)
	      ...
	      if( cis == CBMATCH ){ // 9.11.2013
	        ...
	        if(err==CBSTREAM){
	          ...
	          goto cb_set_cursor_ucs_return;
	            ...
	            ( chr==(**cbs).cf.subrend && intiseven( openpairs ) && (**cbs).cf.doubledelim==1 ) ) ){ // '&', start a new name
	    ...
	    atvalue=0; cb_free_name(&fname); fname=NULL; // allocate_name is at put_leaf and put_name
 	    goto cb_set_cursor_reset_name_index;
...
int cb_save_name_from_charbuf(CBFILE **cbs, cb_name **fname, long int offset, unsigned char **charbuf, int index){
	      ...
	      err = cb_allocate_name(&(*fname), (index+1) ); // moved here 7.12.2013
	      if(err!=CBSUCCESS){  return err; } // 30.8.2013
	      ...
	      // fprintf(stderr,"] index=%i, (**fname).buflen=%i", index, (**fname).buflen);
